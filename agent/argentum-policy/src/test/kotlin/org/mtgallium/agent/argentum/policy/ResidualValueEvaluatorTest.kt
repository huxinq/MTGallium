...
class ResidualValueEvaluatorTest {
    ...
    private fun payload(bias: Double = 0.0, weights: Map<String, Double> = emptyMap()) = LinearWeights(
         bias = bias, weights = weights)

    @Test
    fun `zero residual preserves exact V2 values and its legitimate nonterminal domain`() {
        val zero = ResidualValueEvaluator(payload())
        ...
        assertFailsWith<ValueEvaluationException> { zero.evaluate(information("p0").copy(terminated = true, winnerId = "p0"), "p0") }
        assertFailsWith<ValueEvaluationException> { zero.evaluate(information("p0"), "p1") }
    }

    @Test
    fun `anchored inference clips only after adding residual and missing vocabulary is zero`() {
        val info = information("p0")
        val feature = ValueFeatures.compile(info, "p0").values.entries.first()
        val evaluator = ResidualValueEvaluator(payload(0.25, mapOf(feature.key to 0.5)))
        ...
        assertEquals(1.0, ResidualValueEvaluator(payload(3.0)).evaluate(info, "p0"))
        assertEquals(-1.0, ResidualValueEvaluator(payload(-3.0)).evaluate(info, "p0"))
        val other = information("p0", phase = "POSTCOMBAT_MAIN")
        val newKey = (ValueFeatures.compile(other, "p0").values.keys -
            ValueFeatures.compile(info, "p0").values.keys).first()
        val unseen = ResidualValueEvaluator(payload(0.1, mapOf(newKey to 100.0)))
        assertEquals(MonoRedInformationEvaluator.evaluate(info, "p0") + 0.1, unseen.evaluate(info, "p0"))
    }

    @Test
    fun `ordinary model roundtrip and observers retain the actual calculation`() {
        val keys = ValueFeatures.compile(information("p0"), "p0").values.keys.take(2)
        val first = ResidualValueEvaluator(payload(weights = linkedMapOf(keys[1] to 0.2, keys[0] to 0.1)))
        val second = ResidualValueEvaluator.load(first.model.toJson())
        assertEquals(first.model.toJson(), payload(weights = mapOf(keys[0] to 0.1, keys[1] to 0.2)).toJson())
        assertEquals(first.configurationId, second.configurationId)
        assertNotEquals(first.configurationId, ResidualValueEvaluator(payload(0.1)).configurationId)
        assertEquals(setOf("bias", "weights"), Json.parseToJsonElement(first.model.toJson()).jsonObject.keys)
        val plain = ResidualValueEvaluator.load("""{"bias":0.125,"weights":{}}""")
        assertEquals(MonoRedInformationEvaluator.evaluate(information("p0"), "p0") + 0.125,
            plain.evaluate(information("p0"), "p0"))
        val seen = mutableListOf<ResidualValueEstimate>()
        val observed = first.observedEvaluationBy { _, _, value -> seen += value }
        val source = LeafValueSource.Information(observed)
        assertEquals(first.configurationId, source.invokedEvaluatorConfigurationId)
        assertEquals(first.evaluate(information("p0"), "p0"), observed.evaluate(information("p0"), "p0"))
        assertEquals(1, seen.size)
    }

    @Test
    fun `a supplied model runs at chosen horizons and placements`() {
        val value = ResidualValueEvaluator(LinearWeights(bias = 0.125))
        val invoked = mutableListOf<Int>()
        val observed = value.observedEvaluationBy { info, _, result ->
            invoked += info.observation.turnNumber
            assertEquals(result.anchorValue + 0.125, result.rawScore)
        }
        for (horizon in listOf(3, 17)) {
            invoked.clear()
            val result = search(config().copy(maxPolicyDecisions = horizon), observed)
                .search("p0", belief(FixtureWorld()), 7L)
            assertTrue(invoked.isNotEmpty())
            assertTrue(invoked.all { it == horizon })
            assertEquals(8, result.diagnostics.simulations)
        }
        val custom = object : InformationStateEvaluator {
            override val id = "authored-constant"
            override fun evaluate(information: InformationStateRepresentation, rootPlayer: String) = .2
        }
        assertEquals(custom.id, LeafValueSource.Information(custom).invokedEvaluatorId)
    }

    @Test
    fun `zero residual matches bounded16 search choices values visits and terminal bypass`() {
        val depths = mutableListOf<Int>()
        val evaluator = ResidualValueEvaluator(payload())
        ...
        for (seed in listOf(11L, 29L)) {
            ...
            val v2 = search(config(), MonoRedInformationEvaluator)
    ...
    @Test
    fun `nonfinite residual arithmetic fails`() {
        val info = information("p0")
        val feature = ValueFeatures.compile(info, "p0").values.entries.first { it.value > 1.0 }
        val overflow = ResidualValueEvaluator(payload(weights = mapOf(feature.key to Double.MAX_VALUE)))
        val failure = assertFailsWith<ValueEvaluationException> { overflow.evaluateDetailed(info, "p0") }
        assertEquals(ValueInputError.INFERENCE_NONFINITE, failure.kind)
    }

    private fun config() = InformationSetSearchConfig(simulations = 8, maxPolicyDecisions = 16, leaf = leaf)
    private fun search(config: InformationSetSearchConfig, evaluator: InformationStateEvaluator) = modelTestSearch(config,
        UniformOpponentPolicy, UniformOpponentPolicy, UniformOpponentPolicy, LeafValueSource.Information(evaluator))
    ...
    private fun information(viewer: String, actor: String = "p0", life: Int = 20, depth: Int = 1, phase: String = "PRECOMBAT_MAIN") =
        InformationStateRepresentation(actingPlayerId = actor, observation = PlayerObservationSnapshot(perspectivePlayerId = viewer,
