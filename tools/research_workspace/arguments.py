...
def parser():
    ...
    p.add_argument('attempt', type=Path)
    p.add_argument('--output', type=Path, required=True)
    p = command('audit', 'Authenticate supported retained populations and completion stages without replay, fitting or launch')
    p.add_argument('run', type=Path, help='Frozen attempt or native run directory')
    build_option(p)
    p.add_argument('--deck', type=Path, help='Retained deck for a native factual-study run; attempts supply deck.json')
    p.add_argument('--identity')
    p.add_argument('--timeout', type=int, default=180, help='Total audit wall-time cap in seconds (1–1800)')
    p.add_argument('--heap-mib', type=int, default=4096, help='Native audit heap cap (256–8192 MiB)')
