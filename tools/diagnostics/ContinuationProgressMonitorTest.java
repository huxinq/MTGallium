...
public class ContinuationProgressMonitorTest {
    ...
    // Serialized legacy fields compile against both the original frozen runtime and newer optional fields.
    static PairedSequentialRule rule(int cap,boolean futility){return ContinuationProgressMonitor.JSON.decodeFromString(
        PairedSequentialRule.Companion.serializer(), "{\"nullPointRate\":0.5,\"targetPointRate\":0.5,\"falsePositiveRate\":0.025,\"falseNegativeRate\":0.025,\"maximumPairs\":"+cap+",\"betFractions\":[0.2,0.5,0.8],\"stopForFutility\":"+futility+"}");}
    public static void main(String[]args){
        for(String plan:List.of("{}","{\"seekSuperiority\":false}"))
            ContinuationProgressMonitor.requireSupportedPlan((kotlinx.serialization.json.JsonObject)ContinuationProgressMonitor.JSON.parseToJsonElement(plan));
        boolean unsupported=false;
        try{ContinuationProgressMonitor.requireSupportedPlan((kotlinx.serialization.json.JsonObject)ContinuationProgressMonitor.JSON.parseToJsonElement("{\"seekSuperiority\":true}"));}
        catch(IllegalStateException e){unsupported=e.getMessage().contains("default continuation only");}
        check(unsupported,"superiority continuation cannot silently use the legacy forecast");
