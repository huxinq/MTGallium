...
existing default-mode parent-linked continuation. It explicitly refuses
`seekSuperiority: true`: that mode uses a different confidence-sequence readout,
which this monitor's legacy stopping forecast does not implement. Use the
superiority runner's authenticated report and progress instead.
Develop and commit it in a separate
